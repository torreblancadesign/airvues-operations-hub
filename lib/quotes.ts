...
type QuoteFields = {
  ...
  "Change Order Estimate Cost"?: unknown;
};
...
export async function getQuoteDetail(quoteId: string): Promise<QuoteDetail> {
  ...
  return {
    ...
    preparedForIds: asStringArray(f["Prepared for"]),
    preparedForNames: asStringArray(f["Client Name"]),
    ...
    changeOrderEstimateCost: formatRollupCost(f["Change Order Estimate Cost"]),
    stories,
